...
export default function Index({gate}:indexProps) {
    ...
    const getData = async () => {
        setLoading(true);
        try {
            const response = await axios.post(route('master.zona-waktu.data'), {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const routeName = isEdit 
            ? route('master.zona-waktu.update', data) as string 
            : route('master.zona-waktu.store') as string;
    ...
    const handleHapus = (e: React.FormEvent) => {
        e.preventDefault();
        destroy(route('master.zona-waktu.destroy', data), {
            preserveScroll: true,
            onSuccess: (e) => {
                setHapus(false);
                alertApp(e);
