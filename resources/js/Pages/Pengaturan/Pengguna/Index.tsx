...
export default function Index({gate}:indexProps) {
    ...
    const getData = async () => {
        setLoading(true);
        try {
            const response = await axios.post(route('pengaturan.pengguna.data'), {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const routeName = isEdit 
            ? route('pengaturan.pengguna.update', data) 
            : route('pengaturan.pengguna.store') as string;
    ...
    const handleHapus = (e: React.FormEvent) => {
        e.preventDefault();
        destroy(route('pengaturan.pengguna.destroy', data), {
            preserveScroll: true,
            onSuccess: (e) => {
                setHapus(false);
                alertApp(e);
